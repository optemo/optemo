class Translation < ActiveRecord::Base
  ...
  def self.cache_product_translations
    ...
    CachingMemcached.cache_lookup("TranslationProductType#{I18n.locale}#{Session.product_type}") do
      I18n::Backend::ActiveRecord::Translation.select("`key`, `value`").where(locale: I18n.locale).where("`key` REGEXP ?", (ProductCategory.get_subcategories(Session.product_type) + [Session.landing_page, Session.product_type]).join("|")).inject({}) do |h,f|
        h[f["key"]] = f["value"]
        h
      end
    end
