...
namespace :scrape_extra do
  ...
  task :copy_compatibilities => :init do
    ...
    matching_sets.reject{|x| x.size < 2}.each do |set|
      addme = []
      set.each do |id|
        compats = Compatibility.valid.find_all_by_accessory_id_and_accessory_type(id, Session.product_type)
        addme += compats.collect{|x| [x.product_id,x.product_type]}
        addme.reject!{|x| x.nil? or x[0].nil? or x[1].nil?}
      end
      addme.each do |compat|
        set.each do |id|
          create_uniq_compatibility(id, Session.product_type, compat[0], compat[1])
        end
      end
    end
  end
  ...
  task :moredata_lex  => :data_init do
    ...
    links.each_with_index do |link, index|
      ...
      ps = find_matching_product [clean_specs['mpn']], [clean_specs['model']], Session.product_type,[] # This is broken, check find_matching_product signature
      if ps.length == 1
  ...
  task :download_pix => :pic_init do
    ...
    failed = []
    todo = Session.product_type.find_all_by_imagesurl(nil)
