namespace :refills do
  ...
  task :scrape_new do
    puts "Implement Me"
    #@logfile = File.open("./log/scrape/refills/#{just_alphanumeric($retailers.first.name)}_#{Session.product_type}.log", 'w+')
    ...
    #  announce "Will scrape #{ids.count} #{Session.product_type}s from #{retailer.name}, #{scraped_ids.count} already exist"
    #  
