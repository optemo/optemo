...
#define LIBMYSQLCPPCONN "libmysqlcppconn"
...
#define ensure(msg, stmt)				do {total_tests++;if(!(stmt)){printf("Error! line=%d: %s\n",__LINE__,msg);total_errors++;}} while (0)
#define ensure_equals(msg, op1, op2)	do {total_tests++;if((op1)!=(op2)){printf("Error! line=%d: %s\n",__LINE__,msg);total_errors++;}}while(0)
...
{
//	return new MySQL_Connection(host, port, user, pass);
	try {
		return dm.getConnection(std::string(LIBMYSQLCPPCONN));
	} catch (sql::DbcException *e) {
		printf("sql::DbcException caught during connect\n");
		throw;
	}
}
/* }}} */
...
static bool populate_test_table_PS(sql::Connection * conn)
{
	...
	std::auto_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("CREATE TABLE test_function (a integer, b integer, c integer default null)"));
	ensure("stmt3 is NULL", stmt3.get() != NULL);
	stmt3->executeUpdate();

	std::auto_ptr<sql::PreparedStatement> stmt4(conn->prepareStatement("INSERT INTO test_function (a,b,c) VALUES(1, 111, NULL)"));
...
static bool populate_TX_test_table_PS(sql::Connection * conn)
{
	...
	std::auto_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("CREATE TABLE test_function_tx (a integer, b integer, c integer default null) engine = innodb"));
	ensure("stmt3 is NULL", stmt3.get() != NULL);
	stmt3->executeUpdate();

	std::auto_ptr<sql::PreparedStatement> stmt4(conn->prepareStatement("INSERT INTO test_function_tx (a,b,c) VALUES(1, 111, NULL)"));
	ensure("stmt4 is NULL", stmt4.get() != NULL);
	stmt4->executeUpdate();
...
static void test_connection_0(sql::Connection * conn)
{
	...
	try {
		...
		snprintf(buff, sizeof(buff), "KILL %d", rset1->getInt(0));
...
static void test_statement_3(sql::Connection * conn, sql::Connection * conn2)
{
	...
	try {
		...
		try {
			...
			printf("ERR: sql::DbcException caught\n");
			delete e;
			total_errors++;
		}
...
static void test_result_set_0(sql::Connection * conn)
{
	...
	try {
		...
		ensure("isFirst", result->isFirst() == false);

		ensure("isLast", result->isLast() == false);
	} catch (sql::DbcException *e) {
		printf("ERR: Caught sql::DbcException at %s::%d\n", __FUNCTION__, __LINE__);
		delete e;
		total_errors++;
...
static void test_result_set_3(sql::Connection * conn)
{
	...
	try {
		...
		ensure_equals("sql::Connection differs", conn, stmt1->getConnection());
		...
		ensure("Table not empty after delete", rset2->getInt(0) == 0);
		...
		int count_full_after = rset3->getInt(0);
		...
		ensure("Table not empty after delete", rset4->getInt(0) == 0);
...
static void test_result_set_4(sql::Connection * conn)
{
	...
	try {
		...
		ensure("Rollback didn't work", rset3->getInt(0) == 0);
		ensure("res3 has more rows ", rset3->next() == false);
		...
		ensure("res5 is empty", rset5->next() != false);
		ensure("Table empty after delete", rset5->getInt(0) == count_full_before);
		ensure("res5 has more rows ", rset5->next() == false);
...
static void test_result_set_check_out_of_bound(sql::ResultSet *rset1)
{
	...
	ensure("res1 has more rows ",rset1->getInt(0) == 1);
	ensure("res1 has more rows ",rset1->getInt("count of rows") == 1);

	ensure("res1 has more rows ",rset1->getDouble(0) - 1 < 0.1);
	ensure("res1 has more rows ",rset1->getDouble("count of rows") - 1 < 0.1);

	ensure("res1 has more rows ",rset1->getString(0) == std::string("1"));
	ensure("res1 has more rows ",rset1->getString("count of rows") == std::string("1"));

	ensure("c is not null", rset1->isNull(0) == false);
...
static void test_result_set_8(sql::Connection * conn)
{
	...
	try {
		...
		ensure_equals("sql::Connection differs", conn, stmt0->getConnection());
		...
		/* Let's delete and then rollback */
		ensure("Deleted less rows", stmt5->executeUpdate() == count_full_before);
		...
		ensure("Rollback didn't work", rset4->getInt(0) == 0);
		ensure("res4 has more rows ", rset4->next() == false);
		...
		ensure("Rollback didn't work", rset5->getInt(0) == 0);
		ensure("res5 has more rows ", rset5->next() == false);
...
static void test_result_set_10(sql::Connection * conn)
{
	...
	try {
...

		{
			...
			ensure("Wrong data", rset1->getInt(0) == 42);
			ensure("res1 has more rows ", rset1->next() == false);
...
static void test_general_1(sql::Connection * conn)
{
	...
	try {
		...
		ensure("Empty result set", rset1->next());
		ensure("Wrong data",rset1->getString(1) == std::string("The answer is 42"));
		ensure("Wrong data",rset1->getString("name") == std::string("The answer is 42"));
		ensure("Non-Empty result set", rset1->next() == false);
...
static void test_prep_statement_0(sql::Connection * conn)
{
	...
	try {
		...
		/* Execute without fetching the result set. The connector should clean the wire */
		{
			...
			stmt->setString(2, "Hello MySQL");
			stmt->setDateTime(3, "2006-11-10 16:17:18");
			stmt->execute();
		}


		/* Bind one parameter less than needed - NULL should be sent to the server */
		{
			std::auto_ptr<sql::PreparedStatement> stmt(conn->prepareStatement("SELECT ?, ?, ?"));
			stmt->setInt(2, 42);
			stmt->setString(0, "Hello MySQL");
			...
			ensure("Incorrect value for col 1", rset->getInt(1) == 0 && true == rset->wasNull());

			ensure("Incorrect value for col 2", rset->getString(0) == std::string("Hello MySQL") && false == rset->wasNull());

			ensure("Incorrect value for col 2", rset->getInt(2) == 42 && false == rset->wasNull());
			ensure("Incorrect value for col 2", rset->getString(2) == std::string("42") && false == rset->wasNull());
		}
		...
		/* try clearParameters() call */
		{
			...
			/* Step 1 */
			{
				...
				ensure("Incorrect value for col 2", rset->getString(0) == std::string("") && true == rset->wasNull());

				ensure("Incorrect value for col 2", rset->getInt(2) == 0 && true == rset->wasNull());
				ensure("Incorrect value for col 2", rset->getString(2) == std::string("") && true == rset->wasNull());
			...
			/* Step 2 */
			{
				...
				ensure("Incorrect value for col 0", rset->getString(0) == std::string("Hello MYSQL") && false == rset->wasNull());

				ensure("Incorrect value for col 2", rset->getInt(2) == 42 && false == rset->wasNull());
				ensure("Incorrect value for col 2", rset->getString(2) == std::string("42") && false == rset->wasNull());

				ensure("Incorrect value for col 3", rset->getDouble(3) == 1.25 && false == rset->wasNull());
			}
		}
...
int main(int argc, const char **argv)
{
	...
	const std::string pass(argc >=5? argv[4]:"");
