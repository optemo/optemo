module ImageHelper
  ...
  def filename_from_id id, sz=''
    ...
    return "public/system/#{Session.product_type}/#{id}#{connect}#{sz}.jpg"
  end
  ...
  def record_pic_stats(recset)
    ...
    recset.each do |record|
      # rec = record
      # rec = Product.find_by_id_and_product_type(record, Session.product_type)
      @@size_names.each do |sz|
    ...
    id_and_url_hash.each do |id, url|
      begin
        ...
        unless url.blank? or file_exists_for(id)
          while not url.empty?
            oldurl = url.shift
            newurl = download_image(oldurl, "system/#{Session.product_type}", "#{id}.jpg")
            if(newurl.nil?)
              failed << id 
