...
module AmazonScraper
  ...
  def scrape_best_offer asin, region, nokocache=nil
    nokocache = Nokogiri::HTML(open_cache(curr_retailer(region))) unless nokocache
    lowoffer = nil
    item = nokocache.css("item").reject{|x| x.css('asin').content != local_id}.first
    if item
      bestprice ||= item.css('offersummary/lowestnewprice')
      offers = item.css('offers/offer')
      offers.each do |o| 
        price = o.css('offerlisting/price')
        #TODO merchant?
        if(price == bestprice)
          lowoffer = o
          break
  ...
  def offer_to_atthash offer, asin, region
    atts = {}
    atts['pricestr'] = offer.css('offerlisting/price/formattedprice').to_s
    
    if offer.css('offerlisting/price/formattedprice').to_s == 'Too low to display'
        atts['toolow']   = true
        atts['stock'] = false
        #TODO atts['pricestr'] = scrape_hidden_prices(asin,region)
    else
        atts['toolow']   = false
        atts['pricestr'] = offer.css('offerlisting/price/formattedprice')
        atts['stock']    = true
    end
    
    atts['availability'] = offer.css('offerlisting/availability')
    atts['availability'] = 'In stock' if (atts['availability'] || '').match(/out of stock/i) 
   
    atts['merchant']     = offer.css('merchant/merchantid')
    
    atts['url'] = id_to_sponsored_link(asin, region, atts['merchant'])
    atts['iseligibleforsupersavershipping'] = offer.css('offerlisting/iseligibleforsupersavershipping')
    debugger
    return atts
  end
  ...
  def refresh_cache(retailer)
    ...
    merchant_searchstring = get_merchant_str(retailer) || 'All'
    current_page = 1
    loop do
      begin
        res = Amazon::Ecs.item_search('',:browse_node => $browse_node_id, :search_index => $search_index,\
        :condition => 'New', :merchant_id => merchant_searchstring, :country => retailer.region.intern, \
        :response_group => 'Offers', :item_page => current_page)
        be_nice_to_amazon
      rescue Exception => exc
        report_error "Couldn't download Offers for page #{current_page}"
        report_error "#{exc.class.name} #{exc.message}"
      else
        totalpages ||= res.total_pages if res
        nokodoc = res.doc.to_html
        cachefile.puts("#{nokodoc}") if res        
      end
